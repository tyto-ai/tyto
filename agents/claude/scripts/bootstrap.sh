...

{
  ...
  echo "channel: ${CHANNEL}, version: ${MEMSO_VERSION}-${PLUGIN_VERSION}"
...
if [[ -n "${MEMSO_BINARY_OVERRIDE:-}" ]]; then
  if [[ ! -f "${MEMSO_BINARY_OVERRIDE}" ]]; then
    log_err "[memso] MEMSO_BINARY_OVERRIDE is set but '${MEMSO_BINARY_OVERRIDE}' does not exist"
    exit 0
  fi
  log "[memso bootstrap] Binary ready (override: ${MEMSO_BINARY_OVERRIDE})"
  exit 0
fi
...
case "${OS}" in
  Linux)
    case "${ARCH}" in
      ...
      *) log_err "[memso] Unsupported architecture: ${ARCH}"; exit 0 ;;
    esac
    ;;
  Darwin)
    case "${ARCH}" in
      ...
      *) log_err "[memso] Unsupported architecture: macOS ${ARCH}"; exit 0 ;;
    esac
    ;;
  MINGW* | MSYS* | CYGWIN*)
    ...
    ;;
  *)
    log_err "[memso] Unsupported OS: ${OS}"
    exit 0
    ;;
esac
...
if [[ "${CHANNEL}" == "dev" ]]; then
  ...
  if [[ -z "${REMOTE_SHA}" ]]; then
    log_err "[memso] Could not reach dev release - using existing binary if available"
    log "[memso bootstrap] Binary ready (offline, SHA unknown)"
    exit 0
  fi
  ...
  if [[ -f "${VERSION_FILE}" ]]; then
    ...
    if [[ "${installed}" == "${COMPOSITE}" ]]; then
      log "[memso bootstrap] Binary ready (v${COMPOSITE})"
      exit 0
    fi
...
# ---------------------------------------------------------------------------
log_err "[memso] Downloading ${ARTIFACT} (${VERSION_LABEL})..."

mkdir -p "${CLAUDE_PLUGIN_DATA}"
ARCHIVE="${CLAUDE_PLUGIN_DATA}/${ARTIFACT}"

if ! curl -fsSL "${DOWNLOAD_URL}" -o "${ARCHIVE}"; then
  log_err "[memso] Download failed: ${DOWNLOAD_URL}"
  exit 0
fi
...
case "${ARTIFACT}" in
  *.tar.gz)
    if [[ -f "${BINARY}" ]] && ! mv -f "${BINARY}" "${BINARY}.backup" 2>/dev/null; then
      log_err "[memso] Binary is in use - stop all Claude Code sessions to apply the update"
      log "[memso bootstrap] Update deferred (binary in use)"
      exit 0
    fi
